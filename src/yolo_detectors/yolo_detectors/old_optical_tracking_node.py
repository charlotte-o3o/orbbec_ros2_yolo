...
class OpticalTrackingNode(Node):
            ...
            """if is_held:
                ...
                if object_center_meters is not None:
                    ...
                    if len(self.object_position_history) >= 2:
                        t_prev, pos_prev = self.object_position_history[-2]
                        t_curr, pos_curr = self.object_position_history[-1]
                        dt_inst = t_curr - t_prev
                        if dt_inst > 0:
                            vx_inst = (pos_curr[0] - pos_prev[0]) / dt_inst
                            vy_inst = (pos_curr[1] - pos_prev[1]) / dt_inst
                            vz_inst = (pos_curr[2] - pos_prev[2]) / dt_inst
                            self.hand_velocity_history.append((vx_inst, vy_inst, vz_inst))  """  
            if is_held:
                self.lost_contact_count = 0
                self.last_valid_bbox = bbox
                self.last_valid_object_center_m = object_center_meters
                ...
                # Wrist position is far more robust than the object's during fast motion
                # (motion blur regularly makes YOLO miss the small object, but rarely the wrist).
                wrist_pos = self.get_wrist_position_3d(wx, wy, cv_depth_image)
                if wrist_pos is not None:
                    self.wrist_position_history.append((curr_stamp, wrist_pos))
                    if len(self.wrist_position_history) > self.max_history_len:
                        self.wrist_position_history.pop(0)

                    if len(self.wrist_position_history) >= 2:
                        t_prev, pos_prev = self.wrist_position_history[-2]
                        t_curr, pos_curr = self.wrist_position_history[-1]
                        dt_inst = t_curr - t_prev
                        if dt_inst > 0:
                            vx_inst = (pos_curr[0] - pos_prev[0]) / dt_inst
                            vy_inst = (pos_curr[1] - pos_prev[1]) / dt_inst
                ...
                if self.lost_contact_count >= self.max_lost_contact_frames:
                    release_speed = 0.0
                    peak_vx, peak_vy, peak_vz = 0.0, 0.0, 0.0

                    if self.hand_velocity_history:
                        ...
                        peak_vx = float(np.mean([s[0] for s in top_samples]))
                        peak_vy = float(np.mean([s[1] for s in top_samples]))
                        peak_vz = float(np.mean([s[2] for s in top_samples]))
                        release_speed = float(np.sqrt(peak_vx**2 + peak_vy**2 + peak_vz**2))
                    ...
                    if release_speed >= self.throw_min_release_speed:
                        ...
                        self.last_release_velocity_cam = (peak_vx, peak_vy, peak_vz)
                        self.get_logger().info(
                            f"THROW DETECTED (peak speed: {release_speed:.2f} m/s, from wrist tracking). "
                            f"Last position: {self.last_valid_object_center_m}"
                        )
                    else:
                        self.tracking_state = 'IDLE'
                        self.get_logger().info(
                            f"Contact lost confirmed but peak speed insufficient ({release_speed:.2f} m/s) - reverting to IDLE."
                        )
                    ...
                    self.hand_velocity_history.clear()

        return throw_triggered

    def init_optical_flow_roi(self, cv_depth_image):
        """
        ...
        # Récupération de la vitesse du bras au moment du lâcher
        if hasattr(self, 'last_release_velocity_cam') and self.last_release_velocity_cam is not None:
            ...
            # Transformation de la vitesse de la caméra vers le repère robot
    ...
    def process_thrown_object(self, curr_gray, cv_depth_image, dt, annotated_image, curr_stamp):
        ...
        # 1. Condition de sortie : Timeout global (2s max de vol)
        ...
        # 2. Estimation de profondeur avec filtre anti-saut aberrant
        depth_estimate = self.estimate_object_depth_in_roi(
            cv_depth_image, self.flow_roi, self.flow_arm_mask, self.flow_object_depth
        )
        if depth_estimate is not None:
            if abs(depth_estimate - depth_prev) < 1.0:  # Filtre les sauts de >1m par frame
        ...
        # 3. Condition de sortie : Objet trop près de la caméra
        ...
        has_pixel_movement = abs(vx_px) > 0.5 or abs(vy_px) > 0.5
        has_depth_movement = abs(depth_curr - depth_prev) > 0.01

        time_since_throw = curr_stamp - self.throw_start_time if self.throw_start_time else 0.0
        flow_speed = np.sqrt(vx_rob**2 + vy_rob**2 + vz_rob**2)
        min_trusted_flow_speed = 0.3

        # Période de grâce de 150ms : on ignore la caméra, on fait confiance à l'élan de la main
        if (time_since_throw >= self.velocity_grace_period_s and min_trusted_flow_speed < flow_speed <= self.max_valid_speed):
            ...
            self.get_logger().warn(f"Rejected implausible flow velocity: {flow_speed:.2f} m/s")
        ...
        self.get_logger().info(
            ...
            f"KF Velocity (Robot): vx={float(self.kf_x[3]):.2f}, vy={float(self.kf_x[4]):.2f}, vz={float(self.kf_x[5]):.2f} | "
            f"Flow Vel measured: vx={vx_rob:.2f}, vy={vy_rob:.2f}, vz={vz_rob:.2f}"
        )

        # 4. Condition de sortie : Impact Sol (caméra à ~1m donc sol à ~ -1.0m)
        if time_since_throw > 0.15 and kf_z_rob <= -0.85: 
            self.get_logger().info(
                f"End of throw (Ground reached: Z_robot={kf_z_rob:.2f}m). Reverting to IDLE."
            )
            self.tracking_state = 'IDLE'
            return

        # 5. ASSERVISSEMENT DE LA ROI SUR LE FILTRE DE KALMAN (Remplacement de la propagation naïve)
        ...
        if uv_kf is not None:
            ...
            # Recadrage de la ROI autour du point projeté par le KF
            x1, y1, x2, y2 = self.flow_roi
            roi_w = x2 - x1
            roi_h = y2 - y1

            new_x1 = max(0, min(u - roi_w // 2, w_frame - 1))
            new_y1 = max(0, min(v - roi_h // 2, h_frame - 1))
            new_x2 = max(new_x1 + 1, min(new_x1 + roi_w, w_frame))
            new_y2 = max(new_y1 + 1, min(new_y1 + roi_h, h_frame))
            
            self.flow_roi = (new_x1, new_y1, new_x2, new_y2)

            if self.annotations_mode:
                # Dessin de la ROI centrée sur le KF
            ...
            # L'objet est passé derrière la caméra
    ...
    def synchronized_callback(self, color_msg, depth_msg, yolo_objects, yolo_poses):
        try: 
            ...
            if throw_triggered:
                ...
                self.init_optical_flow_roi(cv_depth_image)
