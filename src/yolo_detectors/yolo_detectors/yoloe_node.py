...
class YoloENode(Node):
    ...
    def __init__(self):
        ...
        self.model_path           = self.declare_parameter('model_path',     'weights/yoloe-26s-seg.pt').value
        ...
        self.get_logger().info("*** YOLOE Node Launched successfully ***")

        # CvBridge converter initialization
        ...
        # YOLOE requires explicit text prompt embeddings (get_text_pe),
        # unlike YOLOWorld where set_classes(names) alone is enough.
        text_embeddings = self.model.get_text_pe(self.custom_classes)
        self.model.set_classes(self.custom_classes, text_embeddings)
        ...
        # Approximate time synchronizer configuration
        ...
        # Callback function for the two synchronized messages
    ...
    def synchronized_callback(self, color_msg, depth_msg):
        try:
            ...
            # 2. 90° rotation (clockwise / counter-clockwise)
            # Change according to your mount: cv2.ROTATE_90_CLOCKWISE or cv2.ROTATE_90_COUNTERCLOCKWISE
            ...
            # YOLOE inference on the rotated image
            ...
            annotated_image = cv_color_image.copy()  # Annotate on the original image
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    # --- Depth extraction on the rotated depth map ---
                    ...
                    # 3. Remap coordinates back to the original frame
                    ...
                    # Compute the 3D position in the native camera frame
                    if distance > 0:
                    ...
                    # Fill in the ROS 2 message (coordinates in the ORIGINAL frame)
                    ...
                    # Draw on the original image for visual verification
            ...
            cv2.imshow("BGR Image with YOLOE", annotated_image)
            cv2.waitKey(1)
